...
#let draw-current(component-name, input) = {
  import "/circetz/components.typ": currarrow
...
#let draw-voltage(input) = {
  let (ab, lr) = if type(input) == array {
    let p = input.first()
    input = input.last()
    (p.find(regex("\^|_")), p.find(regex("<|>")))
  } else {
    (none, none)
  }
  content(
    (
      rel: (0, 0.8em * if ab == "^" { 1 } else { -1 }),
      to: (if ab == "^" { "component.north" } else { "component.south" }),
    ),
    input,
  )
  content(
    (
      rel: (0.4em, 0.6em * if ab == "^" { 1 } else { -1 }),
